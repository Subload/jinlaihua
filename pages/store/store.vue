			...
			<view class="store-swiper">
				<swiper class="swiper" indicator-color="rgba(255,255,255,0.3)" indicator-active-color="#fff" indicator-dots="true" autoplay="autoplay" interval="2000" duration="500">
					<swiper-item v-for="(banner,index) in swiperInfo.info" :key="index">
						<image class="swiper-img" :src="banner.picUrl" mode=""></image>
					</swiper-item>
				</swiper>
			</view>
	...
	export default {
		...
		data() {
			return {
				swiperInfo:{
					info:[{
						picUrl:"/static/store/1@3x.png"
					},
					{
						picUrl:"/static/store/2@3x.png"
					}],
				},
		...
		methods: {
			...
			// 获取商品列表
			getStorePro(){
				this.$API.CommdityAll(this.storeProPage).then(res => {
					uni.hideLoading()
					setTimeout(() => {
